    ...
    for run in range(100):
        ...
        for iteration in range(50):
            ...
            for arc in bestArcs1:
                locFrom=arc[0]
                locTo=arc[1]
                phiM1[locFrom][locTo]=(1-alpha)*phiM1[locFrom][locTo]+alpha/(initSolution['distance']*0.5)
        ...
        #log run
        c.execute('''INSERT INTO RunSum(run,bsiter1,bsiter2,alpha,vehCount, distance)
                     VALUES(?,?,?,?,?,?)''', (run,bsiter1,bsiter2,alpha,bestSolution['vehicleCount'],bestSolution['distance'])) 
    alpha+=0.05
    alpha=round(alpha,2)        
conn.commit()
conn.close()
