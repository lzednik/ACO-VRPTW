...
c.execute('DELETE FROM Run_Summary')
...
for run in range(100):
    ...
    for iteration in range(50):
        ...
        for arc in bestArcs1:
            locFrom=arc[0]
            locTo=arc[1]
            phiM1[locFrom][locTo]=(1-alpha)*phiM1[locFrom][locTo]+alpha/(initSolution['distance']*0.7)
    ...
    #log run
    c.execute('''INSERT INTO Run_Summary(run,vehCount, distance)
                 VALUES(?,?,?)''', (run,bestSolution['vehicleCount'],bestSolution['distance'])) 
